...
func _ready() -> void:
	...
	buttons.assign(NodeExtention.get_children_by_type(self, "Button",true))
	player = NodeExtention.get_child_by_type(NodeExtention.get_root(self),Player,true)
	...
	var player_animated_sprite: AnimatedSprite2D = NodeExtention.get_child_by_type(player, "AnimatedSprite2D")
	...
	if(GameState.level_success):
		...
		time_labels.assign(NodeExtention.get_children_by_type(self,"Label",true))
		...
		var rating_rect: TextureRect = NodeExtention.get_child_by_type(self,"TextureRect",true)
