...
class SaveFile:
	"""
	this class's job is to translate between a plain class andd godot.dict
	godot.dict is one of the few types godot can save
	this results in some quirkieness:
		we want two ways to construct this class: one with a dict for laoding and one blank
		because godot classes do not support function overload, one is _init, the other static
	"""
	...
	#static func create_blank() -> SaveFile:
		#var _return: SaveFile = SaveFile.new({SaveFileKeys.LAST_TIMES: [], SaveFileKeys.BEST_TIMES: []})
		#var size: int = SceneLibrary.levels.size()
		#
		#_return.last_times.resize(size)
		#_return.last_times.fill(0)
		#
		#_return.best_times.resize(size)
		#_return.best_times.fill(0)
		#return _return


#class PlayerPref:
	#var video_settings: Array[int]
	#var audio_settings: Array[float]
	#var control_settings: Dictionary[String, Array]
